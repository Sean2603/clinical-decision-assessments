# Bites - Human — Publication history

<!-- cdm-publication:0.1.3 -->
## v0.1.3 — 2026-09-14

- **Content ID:** `bites-human`
- **Content type:** prescribing-pathway
- **Previous published version:** 0.1.2
- **Published version:** 0.1.3
- **Generated:** 2026-09-14T15:32:44.573Z

## Change control

- **Summary:** Added: Regimens — AND; Added: Regimens — AND.
- **Risk:** clarification
- **Selected version increase:** patch
- **Recommended version increase:** patch
- **Changed fields:** 5
- **Override reason:** Not recorded

## Clinical review

- **Status:** clinically-validated
- **Reviewer:** Sean McGrory
- **Reviewer role:** tAPUC
- **Reviewed at:** 2026-09-14T12:00:00.000Z
- **Notes:** Review completed. No additional review comments recorded.

## Validation

- **Publication ready:** Yes
- **Errors:** 0
- **Warnings:** 0

<!-- /cdm-publication:0.1.3 -->

---

<!-- cdm-publication:0.1.2 -->
## v0.1.2 — 2026-08-30

- **Content ID:** `bites-human`
- **Content type:** prescribing-pathway
- **Previous published version:** 0.1.1
- **Published version:** 0.1.2
- **Generated:** 2026-08-30T17:46:46.725Z

## Change control

- **Summary:** Formatting / structure only: clinical Notes; Removed: regimens item 0 › components; Added: regimens item 0 › display Name; plus 19 other changes.
- **Risk:** clinical-wording
- **Selected version increase:** patch
- **Recommended version increase:** minor
- **Changed fields:** 19
- **Override reason:** fixed some medication linking

## Clinical review

- **Status:** clinically-validated
- **Reviewer:** Sean McGrory
- **Reviewer role:** tAPUC
- **Reviewed at:** 2026-08-30T12:00:00.000Z
- **Notes:** Review completed. No additional review comments recorded.

## Validation

- **Publication ready:** Yes
- **Errors:** 0
- **Warnings:** 0

<!-- /cdm-publication:0.1.2 -->
