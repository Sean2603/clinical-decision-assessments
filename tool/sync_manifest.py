...
def canonical_revocations(value: object) -> object:
    ...
    def key(entry: object) -> tuple[str, str, str, str, str, str]:
        ...
        return (str(entry.get("contentType","")), str(entry.get("contentId","")), str(entry.get("action","")),
                str(affected.get("minimum","")), str(affected.get("maximum","")), str(entry.get("reason","")))
    return sorted(value, key=key)

def build_entries(directory: Path, schema_path: Path, version_field: str,
                  existing_entries: dict[str, dict], allow_empty: bool=False) -> list[dict]:
    ...
    for item_path in paths:
        ...
        entries.append({
            "id": item_id, "title": title, "version": version,
            "file": item_path.relative_to(ROOT).as_posix(),
            "schema": schema_path.relative_to(ROOT).as_posix(),
            "sha256": sha256(item_path), "schemaSha256": sha256(schema_path),
            **generated_metadata,
            **{k:v for k,v in previous.items() if k not in {
                "id","title","version","file","schema","sha256","schemaSha256","categoryIds"
            }},
        })
    return entries
...
def build_manifest(current: dict) -> tuple[dict, bool]:
    ...
    for key, settings in COLLECTIONS.items():
        ...
        existing = {
            entry.get("id"): entry for entry in current.get(key, [])
            if isinstance(entry, dict) and isinstance(entry.get("id"), str)
        }
        generated_collections[key] = build_entries(
            settings["directory"], schema_path, settings["versionField"], existing,
    ...
    generated_keys = {
        "schemaVersion","contentVersion","minimumAppVersion","references","glossary",
        "assessmentCategories","assessments","guidelines","scoringTools","bloodPanels",
    ...
    if "emergencyRevocations" in preserved:
        preserved["emergencyRevocations"] = canonical_revocations(preserved["emergencyRevocations"])

    candidate = {
        **preserved,
        "schemaVersion": 3,
        "contentVersion": previous_version,
        "minimumAppVersion": max_semver(str(current.get("minimumAppVersion","0.32.0")), "0.49.0"),
        "references": {"file":"references/references.json","sha256":sha256(REFERENCES_PATH)},
        "glossary": {"file":"glossary/glossary.json","sha256":sha256(GLOSSARY_PATH)},
    ...
    current_semantic = {k:v for k,v in current.items() if k not in {"contentVersion","updatedAt"}}
    candidate_semantic = {k:v for k,v in candidate.items() if k != "contentVersion"}
    ...
    generated = {
        ...
        **{k:v for k,v in candidate.items() if k not in {"schemaVersion","contentVersion"}},
    }
    return generated, semantic_changed

def main() -> None:
    parser = argparse.ArgumentParser(description="Deterministically generate or verify manifest.json.")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--write", action="store_true", help="Write manifest.json. Intended for the CDM Content Manager batch publisher.")
    mode.add_argument("--check", action="store_true", help="Verify manifest.json is already synchronised without writing it.")
    ...
    if not args.write:
        if actual != expected:
            raise SystemExit("manifest.json is not synchronised. Publish through the CDM Content Manager so the manifest is generated once in the controlled batch.")
        print(f"manifest.json verified: deterministic and synchronised; contentVersion={generated['contentVersion']}")
        return
    changed = write_json_if_changed(MANIFEST_PATH, generated)
    print(
        "manifest.json synchronised by controlled writer: "
        f"contentVersion={generated['contentVersion']}, semanticChanged={str(semantic_changed).lower()}, "
        f"fileChanged={str(changed).lower()}, assessments={len(generated['assessments'])}, "
        f"guidelines={len(generated['guidelines'])}, scoringTools={len(generated['scoringTools'])}, "
        f"bloodPanels={len(generated['bloodPanels'])}, medications={len(generated['medications'])}, "
